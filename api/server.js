...
async function fetchParallelMarket() {
  ...
  try {
    // (ملاحظة: sarf-today قد يحتاج User-Agent لتجنب الحظر)
...
async function fetchGoldRates() {
  try {
    ...
    // فلترة النتائج لتشمل العيارات الشائعة فقط
...
app.get('/api/all-rates', async (req, res) => {
  ...
  const results = await Promise.allSettled([
    fetchNBE(),
    fetchBanqueMisr(),
    fetchParallelMarket() // (تفعيل كاشط السوق السوداء)
  ]);
...
// ============ ✅ فحص التشغيل ============
// (Vercel يستخدم هذا الملف كوحدة ولا يحتاج app.listen)
module.exports = app;
